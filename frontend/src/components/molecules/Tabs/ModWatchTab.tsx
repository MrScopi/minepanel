...
export const ModWatchTab: FC<ModWatchTabProps> = ({ serverId, config }) => {
  ...
  const handleViewChangelog = async (provider: ModProvider, entry: ModEntry, label: string) => {
    setChangelogState({ open: true, provider, entry, label, loading: true, segments: [] });
    ...
    try {
      ...
      if (metadata.desiredMcVersion) {
        const filtered = sortNewestFirst(
          await fetchModVersions(provider, entry.ref, { minecraftVersion: metadata.desiredMcVersion, loader: resolvedLoader }),
        );
        const targetVersionId = filtered[0]?.versionId;
        const foundIndex = full.findIndex((version) => version.versionId === targetVersionId);
        targetIndex = foundIndex >= 0 ? foundIndex : 0;
      }

      const rangeVersions = currentIndex === -1 ? [] : targetIndex < currentIndex ? full.slice(targetIndex, currentIndex) : [];

      const segments: ChangelogSegment[] = await Promise.all(
        rangeVersions.map(async (version) => ({
          versionId: version.versionId,
          name: version.name,
          versionNumber: version.versionNumber,
          datePublished: version.datePublished,
          changelog: provider === 'modrinth' ? (version.changelog ?? null) : await fetchCurseforgeChangelog(entry.ref, version.versionId),
        })),
      );

      setChangelogState((prev) => (prev ? { ...prev, loading: false, segments } : prev));
    } catch (error) {
    ...
    <div className="space-y-4">
      ...
      <Dialog open={changelogState?.open ?? false} onOpenChange={(open) => !open && setChangelogState(null)}>
        <DialogContent className="max-h-[80vh] w-[min(92vw,42rem)] overflow-y-auto bg-gray-900 border border-gray-700 text-white">
          ...
          {changelogState?.loading ? (
          ...
          ) : changelogState && changelogState.segments.length === 0 ? (
            ...
            <div className="space-y-4">
              {changelogState?.segments.map((segment) => (
